...
@section('content')
    @php
        ...
        $selectedIssues = old('issue_ids', $coupon->issues->pluck('id')->all());
    @endphp
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="mb-0">تعديل كوبون خصم</h4>
            ...
            <div class="card-body">
                <form action="{{ route('dashboard.coupons.update', $coupon) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $coupon->name) }}" required maxlength="255">
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $coupon->code) }}" required maxlength="100">
                        @error('code')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="number" step="0.01" min="0" class="form-control @error('discount_value') is-invalid @enderror" id="discount_value" name="discount_value" value="{{ old('discount_value', $coupon->discount_value) }}" required>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="number" min="1" step="1" class="form-control @error('total_usage_limit') is-invalid @enderror" id="total_usage_limit" name="total_usage_limit" value="{{ old('total_usage_limit', $coupon->total_usage_limit) }}" placeholder="اتركه فارغاً لعدم التحديد">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="number" min="1" step="1" class="form-control @error('per_user_usage_limit') is-invalid @enderror" id="per_user_usage_limit" name="per_user_usage_limit" value="{{ old('per_user_usage_limit', $coupon->per_user_usage_limit) }}" placeholder="اتركه فارغاً لعدم التحديد">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="datetime-local" class="form-control @error('starts_at') is-invalid @enderror" id="starts_at" name="starts_at" value="{{ old('starts_at', $coupon->starts_at->format('Y-m-d\TH:i')) }}" required>
                            @error('starts_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-4">
                            ...
                            <input type="datetime-local" class="form-control @error('ends_at') is-invalid @enderror" id="ends_at" name="ends_at" value="{{ old('ends_at', $coupon->ends_at->format('Y-m-d\TH:i')) }}" required>
                            @error('ends_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('applies_to') is-invalid @enderror" id="applies_to" name="applies_to" required>
                            @foreach ($appliesLabels as $value => $label)
                                <option value="{{ $value }}" @selected(old('applies_to', $coupon->applies_to) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('applies_to')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div id="section-specific-products" class="mb-4 d-none">
                        ...
                        <select name="product_ids[]" multiple class="form-select @error('product_ids') is-invalid @enderror @error('product_ids.*') is-invalid @enderror" style="min-height: 180px;">
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" @selected(in_array($product->id, $selectedProducts))>{{ $product->name }} ({{ $product->sku }})</option>
                            @endforeach
                        </select>
                        @error('product_ids')
                    ...
                    <div id="section-specific-issues" class="mb-4 d-none">
                        ...
                        <select name="issue_ids[]" multiple class="form-select @error('issue_ids') is-invalid @enderror @error('issue_ids.*') is-invalid @enderror" style="min-height: 180px;">
                            @foreach ($issues as $issue)
                                <option value="{{ $issue->id }}" @selected(in_array($issue->id, $selectedIssues))>{{ $issue->title }}</option>
                            @endforeach
                        </select>
                        @error('issue_ids')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
