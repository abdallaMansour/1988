...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-body">
                <form id="issue-edit-form" action="{{ route('dashboard.issues.update', $issue) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('crime_type') is-invalid @enderror" id="crime_type" name="crime_type" required>
                            <option value="" disabled @selected(! old('crime_type', $issue->crime_type))>اختر نوع الجريمة</option>
                            @foreach (\App\Models\Issue::crimeTypeOptions() as $type)
                                <option value="{{ $type }}" @selected(old('crime_type', $issue->crime_type) === $type)>{{ $type }}</option>
                            @endforeach
                        </select>
                        @error('crime_type')
                            <div class="invalid-feedback">{{ $message }}</div>
...
@section('page-js')
    ...
    @include('dashboard.partials.rich-text-editor-scripts', [
        ...
        'initialHtml' => old('details', $issue->details ?? ''),
